...
lda_pred <- predict(ldaFit,trainX)

cm_lda = confusionMatrix(lda_pred, as.factor(trainy))

lda = list(classifier = ldaFit, confusionMatrix = cm_lda)
lda
...
nscGrid = expand.grid(.threshold = 0:8)
set.seed(476)

nsc = train(trainX, trainy, method = "pam", tuneGrid = nscGrid, preProc = c("center","scale"))

nsc_pred <- predict(nsc,trainX)

cm_nsc = confusionMatrix(nsc_pred, as.factor(trainy))
nsc = list(classifier = nsc, confusionMatrix = cm_nsc)
nsc
```
...
set.seed(476)
glmnet = train(trainX, trainy, method = "glmnet", tuneGrid = glmnGrid, preProc = c("center","scale"))

glm_pred <- predict(glmnet,trainX)

cm_glm = confusionMatrix(glm_pred, as.factor(trainy))
glm = list(classifier = glm, confusionMatrix = cm_glm)
glm
```
